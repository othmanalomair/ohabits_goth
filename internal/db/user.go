...
func GetUserWithPassword(db *pgxpool.Pool, userID uuid.UUID) (User, error) {
	...
	err := db.QueryRow(context.Background(), `
        ...
        FROM users
        WHERE id = $1
    `, userID).Scan(&user.ID, &user.Email, &user.Password, &user.DisplayName, &user.AvatarURL, &user.CreatedAt, &user.UpdatedAt)
	if err != nil {
		return User{}, err
	}
	return user, nil
...
func UpdateUserWithPassword(db *pgxpool.Pool, user User, userID uuid.UUID) error {
	_, err := db.Exec(context.Background(), `
		UPDATE users SET email = $1, display_name = $2, avatar_url = $3, password = $4, updated_at = NOW()
		WHERE id = $5
	`, user.Email, user.DisplayName, user.AvatarURL, user.Password, userID)
	if err != nil {
		return err
	}
	return nil
